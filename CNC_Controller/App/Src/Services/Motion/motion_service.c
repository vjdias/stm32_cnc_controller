...
enum {
  MOTION_ACK_OK = 0,
  MOTION_ACK_INVALID = 1,
  MOTION_ACK_QUEUE_FULL = 2,
};
...
static inline void motion_unlock(uint32_t primask) { __set_PRIMASK(primask); }
...
static void motion_refresh_status_locked(void) {
  g_status.queue_depth =
      (uint8_t)(g_queue_count + (g_has_active_segment ? 1u : 0u));
  ...
  for (uint8_t axis = 0; axis < MOTION_AXIS_COUNT; ++axis) {
    ...
    if (g_has_active_segment && total > 0u) {
      ...
      pct = (emitted >= total) ? 100u
                               : (uint8_t)(((uint64_t)emitted * 100u) / total);
    }
...
static proto_result_t
motion_queue_push_locked(const move_queue_add_req_t *req) {
...
static void motion_begin_segment_locked(const move_queue_add_req_t *seg) {
  ...
  for (uint8_t axis = 0; axis < MOTION_AXIS_COUNT; ++axis) {
    ...
    ax->step_high = 0u;
    motion_hw_reset_step(axis);
    motion_hw_set_direction(axis, (uint8_t)((seg->dirMask >> axis) & 0x1u));
    if (total > 0u)
...
static void motion_update_encoders(void) {
  ...
  int16_t delta_z =
      (int16_t)((uint16_t)now_z - (uint16_t)g_encoder_last_raw[AXIS_Z]);
...
const motion_status_t *motion_status_get(void) { return &g_status; }
