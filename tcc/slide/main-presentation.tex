...
% Ativa o layout de título "split" do tema com o banner da UFAL
\titlebackground*{imagens/brasao-ufal.png}

% Caminhos de imagens do TCC
\graphicspath{{../src/Cap03/}{../src/Cap03/imagens/}{../src/Cap03/img/}}

\title[Controle determinístico para CNC]{Controle determinístico para CNC\\[-0.3em]baseado em STM32L475\\com DDA de alta frequência}
  ...
  \begin{itemize}
    ...
    \item Estrutura típica:
      \begin{itemize}
        ...
        \item \textbf{CRC} de 16 bits sobre os bytes 0..37.
...
\begin{frame}{Layout do controlador CNC}
  \centering
  \includegraphics[width=0.60\linewidth]{TCC_CIRCUITO_CONTROLE_COMPLETO.png}
  \vspace{0.5em}
...
\begin{frame}{DDA: geração de passos determinística}
  \centering
  \includegraphics[width=0.5\linewidth]{dda_line_rasterization.png}
  \vspace{0.5em}
...
%--------------------------------------------------------------------
\begin{frame}{Modelagem discreta posição $\rightarrow$ velocidade}
  \begin{itemize}
    \item Saída medida: posição incremental em passos DDA, a \SI{1}{\kilo\hertz}, a partir dos contadores de encoder.
    \item Em cada amostra, o erro de posição alimenta um controlador PI/PD discreto que ajusta a velocidade de referência do DDA.
    \item O termo derivativo é filtrado exponencialmente para reduzir ruído de quantização e vibrações rápidas.
    \item Assim, a posição medida no tempo discreto é convertida em correções de frequência de STEP (\emph{steps/s}) aplicadas ao DDA.
  \end{itemize}
\end{frame}

%--------------------------------------------------------------------
\begin{frame}{Da física à função de transferência}
  \begin{itemize}
    \item Modelo físico (2ª lei de Newton para rotação):
      \[
        J\,\frac{d\omega(t)}{dt} + B\,\omega(t) = T_{\text{motor}}(t).
      \]
    \item Driver TMC5160 converte frequência de STEP em torque aproximado:
      \[
        T_{\text{motor}}(t) \approx K_{\text{sist}}\,f(t).
      \]
    \item Substituindo e normalizando por $B$:
      \[
        \tau = \frac{J}{B},\quad K = \frac{K_{\text{sist}}}{B},
      \qquad
        \tau\,\frac{d\omega(t)}{dt} + \omega(t) = K\,f(t).
      \]
    \item Aplicando a transformada de Laplace (condições iniciais nulas):
      \[
...
%--------------------------------------------------------------------
\begin{frame}{Modelo FOPDT da malha de velocidade}
  \begin{itemize}
    \item Malha de velocidade aproximada por modelo FOPDT em malha aberta:
      \[
        G_v(s) = \frac{K\,e^{-Ls}}{\tau s + 1}.
      \]
    \item Degrau de comando $U$ gera resposta teórica
      \[
        v(t) = K\,U\,\bigl(1 - e^{-(t-L)/\tau}\bigr) H(t-L),
      \]
      derivada via transformada de Laplace.
    \item Parâmetros $K$, $L$ e $\tau$ extraídos de degraus medidos via telemetria
      (patamar de velocidade, atraso e constante de tempo).
    \item Catálogo $(K,L,\tau)$ organizado por eixo e microstep alimenta a síntese dos ganhos do controlador.
  \end{itemize}
\end{frame}

%--------------------------------------------------------------------
\begin{frame}{Síntese PD via Ziegler--Nichols}
  \begin{itemize}
    \item Com o modelo FOPDT identificado, aplica-se Ziegler--Nichols (curva de reação) para um regulador PD ($K_i = 0$):
      \[
        K_p = 1{,}2\,\frac{\tau}{K\,L}, \qquad
        T_d = 0{,}5\,L, \qquad
        K_d = K_p\,T_d.
      \]
    \item Ganhos contínuos $(K_p,K_d)$ convertidos para inteiros com fator de escala $S = 256$:
      \[
        k_p = \operatorname{round}(S\,K_p), \quad
        k_d = \operatorname{round}(S\,K_d).
      \]
    \item Catálogo de ganhos inteiros por eixo e microstep compartilhado entre o firmware STM32 e o simulador em Python.
    \item A mesma estrutura de ganhos é usada na validação numérica e nos experimentos, garantindo consistência entre modelo e implementação.
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusões}
  \begin{itemize}
    \item STM32L475 com DDA a \SI{50}{\kilo\hertz} entrega geração determinística de pulsos e malhas de controle estáveis.
    \item Malhas de posição e velocidade com encoders TMCS-28 e PI/PD inteiros rastreiam bem perfis trapezoidais típicos de CNC.
    \item Protocolo SPI com quadros fixos e CRC de 16 bits garante enlace robusto com a Raspberry Pi.
    \item Roteiro incremental com telemetria facilitou validar timers, SPI, encoders e PID.
    \item Modelo FOPDT e catálogo de ganhos PD viabilizaram sintonia sistemática da malha de velocidade, coerente com a simulação.
  \end{itemize}
\end{frame}
...
\begin{frame}{Trabalhos futuros}
  \begin{itemize}
    ...
    \item Implementar e avaliar outros modelos de controle baeados na correção preditiva do movimento dos motores.
