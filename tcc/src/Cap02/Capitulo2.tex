...
diminui a suscetibilidade a variações temporais (\emph{jitter}) e a perdas de sincronismo, mantendo a
...
de distúrbios para manter a precisão em alta velocidade~\cite{efficient_reference,real_time_interpolators}.
...
servo dedicados~\cite{real_time_interpolators}. No
...
\section{Sincronização Cruzada de Eixos}
...
sincronismo e de contorno. Neste texto, adotamos o termo
\emph{Sincronização Cruzada de Eixos} para englobar o conjunto de
estratégias que, sobre a malha de posição/velocidade por eixo, impõem
coerência temporal do grupo, em linha com abordagens de controle
\emph{cross-coupled} de contorno descritas em~\cite{efficient_reference,real_time_interpolators,koren_cnc_interpolators}.
...
Note-se que a sincronização cruzada não substitui as malhas PID por eixo: ela opera como
